...
//★★★取得指定名稱工作表
$worksheet = $spreadsheet->getSheetByName('orders_detail'); //或 $spreadsheet->->getSheet(2);
...
for($i = 2; $i <= $highestRow; $i++) {
    ...
    $prod_thumbnail = $worksheet->getCell('D'.$i)->getValue();
    $prod_price = $worksheet->getCell('E'.$i)->getValue();
    $prod_choice = $worksheet->getCell('F'.$i)->getValue();
    $prod_qty = $worksheet->getCell('G'.$i)->getValue();
    $prod_subtotal = $worksheet->getCell('H'.$i)->getValue();
    
    //★★★寫入資料
    $sql = "INSERT INTO `orders_detail`(
        `order_id`, `prod_id`, `prod_name`,
        `prod_thumbnail`, `prod_price`, `prod_choice`,
        `prod_qty`, `prod_subtotal`
        ) VALUES (
            '{$order_id}', '{$prod_id}', '{$prod_name}',
            '{$prod_thumbnail}', '{$prod_price}', '{$prod_choice}',
            '{$prod_qty}', '{$prod_subtotal}'
