...
//★★★你的 excel 檔案路徑 (含檔名)
$inputFileName = 'convert.excel/03.articles.xlsx';

//透過套件功能來讀取 excel 檔
$spreadsheet = \PhpOffice\PhpSpreadsheet\IOFactory::load($inputFileName);

//★★★取得指定名稱工作表
$worksheet = $spreadsheet->getSheetByName('articles'); //或 $spreadsheet->->getSheet(2);
...
for($i = 2; $i <= $highestRow; $i++) {
    ...
    //★★★讀取 cell 值
    $arti_id = $worksheet->getCell('A'.$i)->getValue();
    $arti_title = $worksheet->getCell('B'.$i)->getValue();
    $arti_content = $worksheet->getCell('C'.$i)->getValue();
    $arti_img_1 = $worksheet->getCell('D'.$i)->getValue();
    $arti_img_2 = $worksheet->getCell('E'.$i)->getValue();
    $arti_img_3 = $worksheet->getCell('F'.$i)->getValue();
    $arti_img_4 = $worksheet->getCell('G'.$i)->getValue();
    
    //★★★寫入資料
    $sql = "INSERT INTO `articles`(
        `arti_id`, `arti_title`, `arti_content`,
        `arti_img_1`, `arti_img_2`, `arti_img_3`,
        `arti_img_4`
        ) VALUES (
            '{$arti_id}', '{$arti_title}', '{$arti_content}',
            '{$arti_img_1}', '{$arti_img_2}', '{$arti_img_3}',
