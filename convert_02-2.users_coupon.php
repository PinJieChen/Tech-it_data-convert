...
//★★★取得指定名稱工作表
$worksheet = $spreadsheet->getSheetByName('users_coupon'); //或 $spreadsheet->->getSheet(2);
...
for($i = 2; $i <= $highestRow; $i++) {
    ...
    $code = $worksheet->getCell('B'.$i)->getValue();
    $isUsed = $worksheet->getCell('C'.$i)->getValue();
    $content = $worksheet->getCell('D'.$i)->getValue();
    $discount_func = $worksheet->getCell('E'.$i)->getValue();
    $discount = $worksheet->getCell('F'.$i)->getValue();
    $deadline = $worksheet->getCell('G'.$i)->getValue();
    
    
    //★★★寫入資料
    $sql = "INSERT INTO `users_coupon`(
        `user_id`, `code`, `isUsed`,
        `content`, `discount_func`, `discount`,
        `deadline`
        ) VALUES (
            '{$user_id}', '{$code}', '{$isUsed}',
            '{$content}', '{$discount_func}', '{$discount}',
