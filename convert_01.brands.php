...
//★★★你的 excel 檔案路徑 (含檔名)
$inputFileName = 'convert.excel/01.brands.xlsx';

//透過套件功能來讀取 excel 檔
$spreadsheet = \PhpOffice\PhpSpreadsheet\IOFactory::load($inputFileName);

//★★★取得指定名稱工作表
$worksheet = $spreadsheet->getSheetByName('brands'); //或 $spreadsheet->->getSheet(2);
...
for($i = 2; $i <= $highestRow; $i++) {
    ...
    //★★★讀取 cell 值
    $brand_id = $worksheet->getCell('A'.$i)->getValue();
    $brand_name = $worksheet->getCell('B'.$i)->getValue();
    $brand_CH = $worksheet->getCell('C'.$i)->getValue();
    $brand_img = $worksheet->getCell('D'.$i)->getValue();
    
    //★★★寫入資料
    $sql = "INSERT INTO `brands`(`brand_id`,
        `brand_name`, `brand_CH`, `brand_img`
        ) VALUES (
            '{$brand_id}', '{$brand_name}', '{$brand_CH}', '{$brand_img}')";
    $stmt = $pdo->query($sql);
