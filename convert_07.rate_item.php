...
//★★★你的 excel 檔案路徑 (含檔名)
$inputFileName = 'convert.excel/07.rate_item.xlsx';

//透過套件功能來讀取 excel 檔
$spreadsheet = \PhpOffice\PhpSpreadsheet\IOFactory::load($inputFileName);

//★★★取得指定名稱工作表
$worksheet = $spreadsheet->getSheetByName('rate_item'); //或 $spreadsheet->->getSheet(2);
...
for($i = 2; $i <= $highestRow; $i++) {
    ...
    $rate_A_name = $worksheet->getCell('B'.$i)->getValue();
    $rate_B_name = $worksheet->getCell('C'.$i)->getValue();
    $rate_C_name = $worksheet->getCell('D'.$i)->getValue();
    $rate_D_name = $worksheet->getCell('E'.$i)->getValue();
    $rate_E_name = $worksheet->getCell('F'.$i)->getValue();
    
    //★★★寫入資料
    $sql = "INSERT INTO `rate_item`(
        `cate_id`, `rate_A_name`, `rate_B_name`,
        `rate_C_name`, `rate_D_name`, `rate_E_name`
        ) VALUES (
            '{$cate_id}', '{$rate_A_name}', '{$rate_B_name}',
            '{$rate_C_name}', '{$rate_D_name}', '{$rate_E_name}'
            )";
